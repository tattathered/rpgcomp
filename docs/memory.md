...
- **2026-08-16 — CombatCalculator (CR-001 chiusa, CR-006/007 aperte):** CR-001 (attacchi multipli creature) implementata e chiusa. CR-006 (riepilogo esiti critici) implementata ma **esito parziale** in smoke test: da consolidare l'intero workflow di risoluzione (subflow attacco semplice/doppio, critico singolo, primario+secondario) con review UX/QoL. CR-007 (workflow/UX, 6 punti) resta pending. Badge "Attacco N/M" corretto a stili inline (il progetto usa `index.css` hand-written, NON Tailwind JIT). Registrato in roadmap il refactoring Tailwind JIT (item 6).
- **2026-08-15 — Catalogo incantesimi condiviso (unico):** export CSV (voci 9/10) ora legge dal catalogo Firestore unico `catalogs/spellCatalog` (40 liste × 10 = 400), non più dai JSON statici stale (290). Rimosso il fallback su `Tabella-elenco_incantesimi.json` in `magicHelpers` e il componente orfano `SpellCatalogViewer`. `spellCatalogService` usa `catalogs/spellCatalog` con migrazione lazy dal per-GM legacy (il catalogo 400 esistente viene promosso a condiviso). Regole Firestore aggiornate e **deployate**. Verificato: export 40/400.
