...
| FIX-012 | FIX | PNG Standard (ST-3) non visibile nel Roster Attivo subito dopo il salvataggio in campagna (compare solo uscendo/rientrando). Causa: `campaignNpcs`/`campaignCreatures` caricati una sola volta in App.jsx (`loadAll`, fetch one-shot al mount) senza refresh post-save; `NpcCatalogTab` salva su Firestore e fa redirect a 'campaignroster', ma `CampaignRosterManager` rende solo da props stantie. Fix proposta: attivare le sottoscrizioni real-time già presenti ma inutilizzate `subscribeToCampaignNpcs`/`subscribeToCampaignCreatures` in App.jsx (o refetch post-save come `handleSaveCharacter` per i PG). Segnalato 2026-08-14. | Bassa | 🔵 In Analisi (fix proposta — decisione fix/postponi pendente) | req_07 |
